// Minimal background script with proper Chrome extension messaging
...
async function handleStartPost(message: ChromeMessage, sender: chrome.runtime.MessageSender, sendResponse: (response: MessageResponse) => void) {
  ...
  try {
    ...
    const pingResponse = await new Promise<MessageResponse | undefined>((resolve) => {
      const timeout = setTimeout(() => resolve(undefined), 3000);
      
      chrome.tabs.sendMessage(tabId, { type: "PING" }, (response: MessageResponse | undefined) => {
        clearTimeout(timeout);
        if (chrome.runtime.lastError) {
          console.warn("[background] Ping error:", chrome.runtime.lastError.message);
          resolve(undefined);
        } else {
          resolve(response);
        }
    ...
    if (!pingResponse || !pingResponse.ready) {
      console.error("[background] Content script not ready");
      sendResponse({
        success: false,
